<form action="" class="md:w-1/2 space-y-5" wire:submit.prevent='editarVacante'>
        ...
        <div>
            ...
            <x-text-input id="imagen" class="block mt-1 w-full" type="file" accept="image/*" wire:model.live="imagen_nueva"/>
            

            <div class="my-5 w-80"> 
                <x-input-label for="imagen" :value="__('Imagen vacante'.$titulo)" />
                <img src="{{asset('storage/'.$imagen)}}" alt="imagen actual">
            </div>
            <div class="my-5 w-80">
                @if ($imagen_nueva)
                    Imagen Nueva:
                        <img src="{{ $imagen_nueva->temporaryUrl()}}" alt="image">
                @endif  
            </div>
            <x-input-error :messages="$errors->get('imagen_nueva')" class="mt-2" />
        </div>

        <x-primary-button class="">Guardar Cambios</x-primary-button>
</form>
